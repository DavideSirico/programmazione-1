...
int getValue(struct Stack * & s, int i) {
  if (i == 1) return top(s);
  int v = pop(s);
  int r = getValue(s, i-1);
  push(s, v);
  return r;
}

void calcola_aux(struct Stack * &s1, struct Stack * &s2, int l, int n) {
  if (isEmpty(s1)) return;
  int v = pop(s1);
  int v1 = getValue(s2, l-n);
  calcola_aux(s1, s2, l, n+1);
  push(s1, v + v1);
}
