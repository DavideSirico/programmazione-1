...
// Inserire qui sotto la dichiarazione della funzione count_models

double count_models(const node * n, int n_vars);

// Inserire qui sopra la dichiarazione della funzione count_models
...
int main() {
    ...
    std::cout << "A & B := "; printNode(AandB); std::cout << " models = " << count_models(AandB, 4) << std::endl;
    std::cout << "A | B := "; printNode(AorB); std::cout << " models = " << count_models(AorB, 4) << std::endl;
    std::cout << "A & (B | C) := "; printNode(AandBorC); std::cout << " models = " << count_models(AandBorC, 4) << std::endl;
    std::cout << "A | (B & C) := "; printNode(AorBandC); std::cout << " models = " << count_models(AandBorC, 4) << std::endl;
    node *res[10] = {nullptr};
    ...
    for (int i = 0; i < 10; i++) {
        ...
        std::cout << "res[" << i << "] := ";
        printNode(res[i]); std::cout << " models = " << count_models(res[i], 4) << std::endl;
        deleteNode(res[i]);
    }
    deleteNode(A);
    deleteNode(B);
...
// Inserire qui sotto la definizione della funzione count_models

double countModels_aux(const node * n, double max) {
    if (isTrue(n)) {
        return max;
    } else if (isFalse(n)) {
        return 0.0;
    } else {
        return 0.5 * countModels_aux(getThen(n), max) + 
               0.5 * countModels_aux(getElse(n), max);
    }
}

double count_models(const node * n, int n_vars) {
    double max = pow(2.0, n_vars);
    
    return countModels_aux(n, max);
}
// Inserire qui sopra la definizione della funzione count_models
