...
void stackOperator_ric(struct Stack *&s, struct Stack *&r, int);
...
struct Stack * stackOperator(struct Stack *&s) {
    struct Stack * r = initStack();

    stackOperator_ric(s,r,0);
    return r;
}
// Inserire qui sopra la definizione della funzione stackOperator

void stackOperator_ric(struct Stack *&s, struct Stack *&r, int somma) {
    if(!isEmpty(s)) {
        int current = pop(s);
        stackOperator_ric(s,r, somma+current);
        push(s,current);
        push(r,somma+current);
        push(r,current);
    }
}