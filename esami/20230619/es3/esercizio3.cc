...
void convertiListaInMatrice_aux(int **m, Nodo* testa, int i, int j) {
    if(testa == NULL) {
        return;
    }
    if(testa->figlio != NULL) {
        convertiListaInMatrice_aux(m, testa->figlio, i+1, j);
    }
    if(testa != NULL) {
        m[i][j] = testa->valore;
        convertiListaInMatrice_aux(m, testa->prossimo, i, j+1);
    }
}
int** convertiListaInMatrice(Nodo* testa, int righe, int colonne) {
    ...
    convertiListaInMatrice_aux(m, testa, 0, 0);
    return m;
    // ... scrivere qui l'implementazione della funzione "convertiListaInMatrice"
}
